 ...
 */
@Service
public class FuncCustQueryBSImpl extends WtdkCommon implements IFuncCustQueryBS {
	...
	public String query(CustomersQueryVO queryVO, Paginator paginator) {
		...
		String sql = "  select distinct o.CI_CHN_NAME,o.CI_CUST_NO,o.CI_FLST_DT,o.CI_UNIT_CHN_INIL," +
				"o.CI_GOVN_CERT_NO from crt_dcc_cicifci1 o,crt_dcc_SAACNACN s where " +
				"o.CI_CUST_NO = s.sa_cust_no ";
	...
	public String findFundByCustNo(String ciCustNo) {
		String sql = " select * from CRT_DCC_SAACNACN n where n.SA_CUST_NO = '" + ciCustNo + "'";
		...
		for (int i = 0; i < queryList.size(); i++) {
			...
			fundVO.setSaAvlBal(findLostValue(checkString(fund.get("SA_ACCT_NO"))));// 账户余额
			fundVO.setSaCurrCode(findsaCurrName(findBB(checkString(fund.get("SA_ACCT_NO")))));// 币种
	...
	public String query4excel(String ids){
		// 拼接查询语句
		String sql = " select distinct * from crt_dcc_cicifci1 o,crt_dcc_SAACNACN s where " +
				"o.CI_CUST_NO = s.sa_cust_no and o.CI_CUST_NO in ("+IdStrFormat.formatIds(ids)+") ";
		List queryList = publicDao.queryForList(sql, new ArrayList());
		// 获得查询结果
		List<CrtDccCicifci1> resultList = new ArrayList<CrtDccCicifci1>();
		// 进行属性拷贝
