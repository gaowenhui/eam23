 ...
 */
@Service
public class CustomersQueryBSImpl extends WtdkCommon implements ICustomersQueryBS {
	...
	public String query(CustomersQueryVO queryVO, Paginator paginator) {
		// 拼接查询语句
		String sql = " select distinct * from crt_dcc_cicifci1 o where 1=1  ";
		...
		for (int i = 0; i < queryList.size(); i++) {
			...
			tempVO.setCiFlstDt(checkString(tempBO.get("CI_FLST_DT")).trim());
	...
	private List<Object> prepareParam(CustomersQueryVO queryVO) {
		...
		if (queryVO.getStartCiFlstDt() != null
				&& queryVO.getStartCiFlstDt().trim().length() > 0) {
			list.add(queryVO.getStartCiFlstDt());
		}
		// 结束
		if (queryVO.getEndCiFlstDt() != null
				&& queryVO.getEndCiFlstDt().trim().length() > 0) {
			list.add(queryVO.getEndCiFlstDt());
		}
		return list;
	}
	...
	public CrtDccCicifci1 findCiCustByCustNo(String ciCustNo) {
		...
		if (list.size() > 0) {
			...
			cust.setCiFlstDt(checkString(map.get("CI_FLST_DT")).trim());// 建档日期
			...
			cust.setCiEconChar(checkString(map.get("CI_ECON_CHAR")).trim());// 经营性质
	...
	public String findFundByCustNo(String ciCustNo) {
		String sql = " select * from CRT_DCC_SAACNAMT t,CRT_DCC_SAACNACN n where t.SA_ACCT_NO"
				+ " = n.SA_ACCT_NO and n.SA_CUST_NO = '" + ciCustNo + "'";
		...
		for (int i = 0; i < queryList.size(); i++) {
			...
			fundVO.setSaAcctNo(checkString(fund.get("SA_ACCT_NO")).trim()); // 系统基金账号
			fundVO.setSaCustName(checkString(fund.get("SA_CUST_NAME")).trim()); // 客户名称
			fundVO.setSaAvlBal(checkString(fund.get("SA_AVL_BAL")).trim());// 账户余额
			fundVO.setSaCurrCode(findsaCurrName(checkString(fund.get("SA_CURR_CODE")).trim()));// 币种
			fundVO.setSaCustNo(checkString(fund.get("SA_CUST_NO")).trim());// 客户编号
