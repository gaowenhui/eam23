 ...
 * @version V1.0
 */
public class NCEMOtherMatterHeadAction extends CommonAction{
    
	@Autowired
	ICEMEntityFormUcc iCEMEntityFormService;
	...
	private List<StGlbt> stGlbtList = new ArrayList<StGlbt>();
	StJbxx stJbxx;
	...
	public String modifyHead() {
		...
		stGlbtList = cEMEntityFormUccImpl.viewEntityBody(headId);
	...
	public String saveHead() {
		...
		if(StringUtils.isNotEmpty(taskId)){
			workflowutil.putVariables(taskId, "codeType", "stgl_qtsx");
	...
	/**
	 * 保存表体信息（项目名称，申请金额，审定金额）
	 * @return
	 */
	public String saveBody(){
		headId = head.getId();
		stId = head.getStId();
		String[]  fyzl = request.getParameterValues("stGlbt.fyzl");
		...
		if(sqje != null && sqje.length>0){
			for(int i=0;i<sqje.length;i++){
				stGlbt = new StGlbt();
				stGlbt.setCldId(headId);
				stGlbt.setStId(stId);
				stGlbt.setFyzl(fyzl[i]);
				try {
		...
		public String createNum(){
			...
			if(bianhao==null||bianhao.length()==0){
				bianhao = iCodeGenerator.generateCurrentCode("stgl_qtsx", head.getNgbm(), head.getNgbmmc());
	...
	public ICEMEntityFormUcc getICEMEntityFormService() {
		return iCEMEntityFormService;
	}

	public void setICEMEntityFormService(ICEMEntityFormUcc entityFormService) {
		iCEMEntityFormService = entityFormService;
	}
	...
	public List<StGlbt> getStGlbtList() {
		return stGlbtList;
	}

	public void setStGlbtList(List<StGlbt> stGlbtList) {
		this.stGlbtList = stGlbtList;
	}
