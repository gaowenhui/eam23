/***********************************************************
 ËµÃ÷£º¶ÔSELECT²Ù×÷µÄº¯Êý

 ×÷Õß£ºÍõÖÐÌÎ  2004-10-19
***********************************************************/
 	...
 	for(var x = 0;x < objSource.length; x++)
 	{
		...
		if (opt.selected)
		{
			...
			while (y < objTarget.length)
			{
				...
				// Èç¹û objTarget ÖÐÓÐµ±Ç°ÏîµÄ¸¸Àà£¬Ôò²»ÔÙÌí¼Ó
				...
				//Èç¹û objTarget ÖÐÓÐµ±Ç°ÏîµÄ×ÓÀà£¬ÔòÉ¾³ý×ÓÀàÔÙÌí¼Óµ±Ç°Ïî
			...
			if (blnFlag == true)   // Ìí¼Óµ±Ç°Ïî
				fullup_P(objTarget, DelDelimiter(opt.text), opt.value);
		}
	}
	
	//¼ì²é objTarget µÄÑ¡Ïî¸öÊý£¬Èç¹û³¬³ö£¬Ôò´ÓºóÃæÉ¾³ý
	if (intMaxNumber > 0)
	{
...
function CheckMulSelect(obj, maxNum, minNum, txtTitle)
{
  if(obj.multiple)
  {
		...
		{
			alert('"'+txtTitle+'"  ×î¶àÑ¡Ôñ'+maxNum+'Ïî£¡');
			obj.focus();
			return false;
		}
		...
		{
			alert('"'+txtTitle+'"  ×îÉÙÑ¡Ôñ'+minNum+'Ïî£¡');
			obj.focus();
			return false;
		}
