...
function query(){
	...
	url:'<%=basePath%>wtdk/loansCustQuery_query.do'
	}).trigger("reloadGrid");
}
...
function queryMore(){
	...
	if(gr.length==1 && gr!=""){
		...
		var url = '<%=basePath%>wtdk/loansCustQuery_customerInfo.do?ciCustNo='+id;
...
// excel导出
function excelAll(){
    //var urlStr = '<%=basePath%>wtdk/loansCustQuery_query.do';
	var urlStr = 'com.tansun.eam2.wtdk.action.LoansCustQueryAction&com.tansun.eam2.wtdk.vo.CustomersQueryVO';
	var ciChnName = document.getElementById("ciChnName").value;
	var ciCustNo = document.getElementById("ciCustNo").value;
	var startCiFlstDt = document.getElementById("startCiFlstDt").value;
	var endCiFlstDt = document.getElementById("endCiFlstDt").value;
	...
	exportExcel("customer_list", urlStr, params, '贷款客户列表','rn,cb');
}
...
//勾选导出
function excelCheck(){
	var urlStr = '<%=basePath%>wtdk/loansCustQuery_excelCheck.do';
