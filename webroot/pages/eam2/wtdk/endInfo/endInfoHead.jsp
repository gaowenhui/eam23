...
%>
<r:controlfile file="/pages/eam2/wtdk/endInfo/endInfo.ctl.xml"></r:controlfile>
<!-- ==========================工作流属性初始化=========================================== -->
...
$(document).ready(function(){	
	...
	<%if(!inCounterSign){%>
		...
		// ----------------------------------------查看oa流程------------------------------------------
		<e:pc id="queryoa">
			var buttonStr_oastat = '[{"id":"oastat","buttonHandler":"mainWindow.queryOA","buttonIcon":"icon-search","buttonText":"查看OA状态"}]';	                       
			var buttonStr_oaurl = '[{"id":"oaurlquery","buttonHandler":"mainWindow.queryOAurl","buttonIcon":"icon-search","buttonText":"查看OA表单"}]';	                       
		...
		<e:pc id="endInfoList">
			...
			addButtons(jQuery("#gview_endInfoList > .ui-jqgrid-titlebar"),buttonJSonString);
		</e:pc>
	<%}else{%>
		var buttonStr_stztcz1 = '[{"buttonHandler":"mainWindow.viewzhengwen","buttonIcon":"icon-viewFile","buttonText":"查看正文"}]';	                       
		window.parent.prependButtonsFromChildren(buttonStr_stztcz1,"");
	...
	//----------------------------------------设定宽度--------------------------------------------
	jQuery("#file_list").setGridWidth($("#endWhith").width());
	jQuery("#blyj_reason_list").setGridWidth($("#endWhith").width());
	jQuery("#endInfoList").setGridWidth($("#endWhith").width());
	jQuery("#file_huiqianlist").setGridWidth($("#endWhith").width());
	jQuery("#file_huiqian_endlist").setGridWidth($("#endWhith").width());
...
%>
<!-- 工作流表单提交URL设置 -->
<input type="hidden" id="commitUrl" value="<%=request.getContextPath()%>/wtdk/endInfo_saveHead.do"/>
...
<input type="hidden" id="commitDraftUrl" value="<%=request.getContextPath()%>/wtdk/endInfo_saveHead.do"/>
<input type="hidden" id="draftParticipationUrl" value="<%=urlHeader+draftParticipationMethod+".do"%>"/>
<input type="hidden" id="saveDraftUrl" value="<%=request.getContextPath()%>/wtdk/endInfo_saveHead.do"/>
<input type="hidden" id="saveTodoUrl" value="<%=request.getContextPath()%>/wtdk/endInfo_saveHead.do"/>
<!-- 保存后台的URL -->
...
<!-- 传阅 -->
<input type="hidden" name="circulationUrl" value="wtdk/endInfo_modifyHead.do"/>
...
<input type="hidden" name="counterSignUrl" value="wtdk/endInfo_modifyHead"/>
<input type="hidden" name="actionName" value="<%=actionPath%>_todo"/>
<input type="hidden" name="namespace" value="<%=actionSpaceName%>"/>

<br><div align="center">
	<span class="STYLE7 STYLE1">中国建银投资有限责任公司委代账务审批<%@ include file="/pages/rdp4j/workflow/task/workflow_title_include.jsp"%></span>
</div>
<!-- ==========================工作流信息项======================================= -->
...
<table class="newtable" width="100%" border="0" align="center"
	cellpadding="0" cellspacing="0" id="endWhith">
	<tr>
	...
	<tr>
		...
		<td class="td_zxy02"><s:textfield name="head.sheetId" id="bianhao" theme="simple" disabled="true" cssClass="input" /></td>
	...
	<tr>
		...
		<td class="td_zxy02"><s:textfield name="head.ngrmc" disabled="true" id="ngrmc" maxlength="10" cssClass="input"></s:textfield></td>
...
jQuery("#endInfoList").jqGrid({
	url:"<%=basePath%>wtdk/endInfo_findBody.do?headId="+headId,
	datatype: "json",
	mtype:"POST",
   	colNames: ['id','系统基金账号','基金客户编号','基金客户名称','基金账户余额','贷款账户余额','基金结余'],
	colModel: [
		{name: 'id',index: 'id',align: "center",hidden:true },
		{name: 'fundAccount',index: 'fundAccount',align: "center" },
		{name: 'khbh',index:'khbh',align:'center'},
		{name: 'khmc',index: 'khmc',align: "center"},
		{name: 'zhye',index: 'zhye',align: "center"},
		{name: 'zhye',index: 'zhye',align: "center"},
		{name: 'dkzhye',index: 'kyye',align: "center"}
	],
	rowNum:0,
...
function addAss(){
	...
	window.open('<%=basePath%>wtdk/endInfo_addBody.do?headId='+headId,'终止委托贷款信息',type);
}

// 修改表体
function modify(){
	var gr = jQuery("#endInfoList").jqGrid('getGridParam','selarrrow');
	var headId = document.getElementById('headId').value;
	if(gr.length == 2){
		if(gr[0] == 'undefined'||gr[0] == '' || gr[0] == null){
			...
			var bodyId = gr;
			var type = 'height=300, width=1100,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,left=100,top=150';
			window.open('<%=basePath%>wtdk/endInfo_addBody.do?headId='+headId+'&bodyId='+bodyId,'终止委托贷款信息',type);
		}else{
			alert("请选择一条记录");
			return false;
		}
...
function dbClickQuery(gr){
	...
	var bodyId = gr;
	var type = 'height=300, width=1100,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,left=100,top=150';
	window.open('<%=basePath%>wtdk/endInfo_endQueryDetail.do?headId='+headId+'&bodyId='+bodyId,'终止委托贷款信息',type);
}
...
function delAss(){
	...
	if (gr.length == 0){
		...
		if(confirm("确认删除？")){
			var delurl = '<%=basePath%>wtdk/endInfo_deleteBody.do';
			$.post(delurl,{bodyIds:"" + gr + ""}, function(data){
				jQuery("#endInfoList").trigger("reloadGrid");
			});
		}
	}
}
...
// 提交表单
function tijiao(){
	myform.action = '<%=basePath%>wtdk/endInfo_saveHead.do';
}
...
function createBianhao(){
	var url = "<%=basePath%>wtdk/endInfo_endInfoCode.do";
...
function batchChoose(){
	...
	//window.open('<%=basePath%>wtdk/endInfo_openBatchChoose.do?batch=true','终止委托代业务信息',type);
	window.open('<%=basePath%>wtdk/endInfo_openRelateQuery.do','终止委托代业务信息',type)
}

// 批量选择后回调方法
function callBackBatch(gr){
	var url = "<%=basePath%>wtdk/endInfo_batchChoose.do";
	var headId = document.getElementById('headId').value;
	$.post(url,{funcNo:"" + gr + "",headId:"" + headId + ""}, function(data){
		if(data!=null&&data!=''){
			alert(data);
			jQuery("#endInfoList").trigger("reloadGrid");
		}
...
function callBack(xulies){
	var url = "<%=basePath%>wtdk/endInfo_saveBodysByRelate.do";
	var headId = document.getElementById('headId').value;
	$.post(url,{xulies:"" + xulies + "",headId:"" + headId + ""}, function(data){
		if(data!=null&&data!=''){
			alert(data);
			jQuery("#endInfoList").trigger("reloadGrid");
		}
...
function batchOperate(){
	...
	if(gr.length==0){
		...
		var type = 'height=150,width=700,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no,left=250,top=200';
		window.open('<%=basePath%>wtdk/endInfo_openBatchOperate.do?bodyIds='+gr,'终止委托代业务信息',type);
	}
}

// 批量操作后的回调方法
function callBackOperate(bodyIds,spjg,bz){
	var url = "<%=basePath%>wtdk/endInfo_batchOperate.do";
	var headId = document.getElementById('headId').value;
	$.post(url,{
		bodyIds:"" + bodyIds + "",
		spjg:"" + spjg + "",
...
function checkEAMFormJSMethod(){
	var ids = jQuery("#endInfoList").jqGrid('getDataIDs');
...
function startOAflow(){
	var url = "<%=basePath%>wtdk/endInfo_startOAFlow.do";
	...
	var EAMmsgid = '<%=headId%>';
	var yjwjurl = basePathJs+'wtdk/endInfo_modifyHead.do?headId='+'<%=headId%>';
	var nglx = '发文';
...
function openzhengwen(){
	wordTemplate('bank_Template', document.getElementById("myform"),false,true, "委代账务审批正文"); 
	//修改按钮名称为“处理正文”
...
function viewzhengwen(){
	wordTemplate('bank_Template', document.getElementById("myform"),true,false,"委代账务审批正文"); 
}
//合成正文
function hechengzhengwen(){
	MergeWordStart('manager_gzlxdht','bank_Template','manager_gzlxdbj' , document.getElementById("myform"), "委代账务审批正文",'hechengHead()','hechengBottom()');
