...
$(document).ready(function(){
	...
	addButtons($("#gview_feiyongzhifu > .ui-jqgrid-titlebar"),buttonJSonString2);
	jQuery("#feiyongzhifu").setGridWidth($("#table").width());
	$(window).bind('resize', function(){
		jQuery("#feiyongzhifu").setGridWidth($("#table").width());
	});
});
function addFeiyong1(){
	var gr = jQuery("#feiyongzhifu").jqGrid('getGridParam','selarrrow');
	if(gr.length > 1){
		alert('只能修改一条记录');
	}else if( gr.length == 1 && gr[0]!=null){
		var trackId = document.getElementById("trackId").value;
		var url = '<%=request.getContextPath()%>/zccz/disposeTrack_newAdjunct.do?trackId='+trackId+"&fsxxlx=fyzf&azryId="+gr;
...
<script type="text/javascript">
	jQuery("#feiyongzhifu").jqGrid({
		...
		datatype: "json",
		colNames:['id','姓名','移动电话','固定电话','安置方式','安置费用','实际安置费用','是否签署收条','是否发生仲裁'],
		colModel:[	
			...
			{name:'azrysfqsst',index:'fyzl',editable:true},
			{name:'azrysffszc',index:'fyzl',editable:true}					
		],
		autowidth:true,
		height:'auto',
		multiselect: true,
	...
	function reloadJqGrid(){
		jQuery("#feiyongzhifu").trigger("reloadGrid");
	}
</script>
