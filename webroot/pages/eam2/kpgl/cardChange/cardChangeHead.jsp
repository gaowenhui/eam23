...
<!-- ==========================  权限控制添加     ======================================== -->
<r:controlfile file="/pages/eam2/kpgl/cardChange/cardChange.ctl.xml"></r:controlfile>
<!-- ================================================================================== -->
<!-- ==========================工作流属性初始化=========================================== -->
<%@ include
...
$(document).ready(function(){	
	...
	var buttonJSonString = '[{"buttonHandler":"batchChoose","buttonIcon":"icon-add","buttonText":"增加"},\
							 ...
							 {"buttonHandler":"batchOperate","buttonIcon":"icon-edit","buttonText":"变动"}\
							]';
...
function checkEAMFormJSMethod(){
	var ids = jQuery("#purchase_list").jqGrid('getDataIDs');
...
<!-- =========================================================================== -->
<br><div align="center">
	<span class="STYLE7 STYLE1">中国建银投资有限责任公司卡片变动处理单</span>
</div>
<!-- ==========================工作流信息项======================================= -->
...
<SCRIPT language=JavaScript type=text/JavaScript>
	...
	function send2Erp(){
		...
		var erpurl = erpPathJs + yjwjurl+"?taskId="+taskId;
		var url = "<%=request.getContextPath()%>/jiekou/send2ERP4CHANGE_requestExecute.do?erpurl="+ erpurl;
		//myform.action = url;
...
jQuery("#purchase_list").jqGrid({				
	url:"<%=basePath %>kpgl/cardQuery_findChangeBodyAction.do?headId=<%=headId%>",
	datatype: "json",
	mtype:"POST",
   	colNames: ['id','卡片编号','卡片名称', '资产编号', '存放地点', '使用部门','变动前','变动方式','变动后'],
	colModel: [
		...
		{name: 'beforeValue',index: 'dealmethod',align: "center",sortable:false},
		{name: 'changeway',index: 'changereason',align: "center",sortable:false},
		{name: 'afterValue',index: 'changereason',align: "center",sortable:false}
	],
	multiselect:true,
	rownumbers:true,
	autowidth:true,
...
function addAss(){
	...
	url = url  + "&id=" + new Date();//这个id使每次的url不同，是防止缓存加的时间参数，
	window.open('<%=request.getContextPath()%>/kpgl/cardBody_addBody.do?headId='+headId+'&sheetType='+sheetType,'卡片变动',_g_privateDialogFeatures);
}
...
function batchChoose(){
	...
	window.open(url,'卡片选择',_g_privateDialogFeatures);
}

// 批量选择后的回调方法
function callBackBatch(cardIds){
	var headId = document.getElementById('headId').value;
	var reduceUrl ='<%=basePath%>kpgl/cardBatch_chooseChangeBatch.do';
...
function batchOperate(){
	...
	var _g_privateDialogFeatures = 'height=700, width=1000, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no';
	var url = "<%=basePath%>kpgl/cardBatch_openChangeOperate.do?bodyIds="+gr+"&headId="+headId;
	window.open(url,'卡片选择',_g_privateDialogFeatures);
}
