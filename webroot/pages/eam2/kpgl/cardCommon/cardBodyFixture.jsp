...
<%
	...
	if("true".equals(isModifyBody)){//是否LcCardTemp新增卡片处理单修改
...
<SCRIPT language=JavaScript type=text/JavaScript>
	...
	$(function(){
		...
		%>	
		<%
			if(body!=null&&body.getYzje()!=null&&!"".equals(body.getYzje())){
		%>	
				$("#yzje").val(${body.yzje}.toFixed(2));
		<%	
			}
		%>		
		<%
			if(body!=null&&body.getLjzj()!=null&&!"".equals(body.getLjzj())){
		%>	
				$("#ljzj").val(${body.ljzj}.toFixed(2));
		<%	
			}
		%>	
		<%
			if(body!=null&&body.getJvalue()!=null&&!"".equals(body.getJvalue())){
		%>	
				$("#jvalue").val(${body.jvalue}.toFixed(2));
		<%	
			}
		%>	
		<%
			if(body!=null&&body.getJevalue()!=null&&!"".equals(body.getJevalue())){
		%>	
				$("#jevalue").val(${body.jevalue}.toFixed(2));
		<%	
			}
		%>	
		<%
			if(body!=null&&body.getJcz()!=null&&!"".equals(body.getJcz())){
		%>	
				$("#jcz").val(${body.jcz}.toFixed(2));
		<%	
			}
		%>	
		<%
			if(body!=null&&body.getYzjl()!=null&&!"".equals(body.getYzjl())){
		%>	
				$("#yzjl").val(${body.yzjl}.toFixed(2));
		<%	
			}
		%>	
		<%
			if(body!=null&&body.getJczl()!=null&&!"".equals(body.getJczl())){
		%>	
				$("#jczl").val(${body.jczl}.toFixed(2));
		<%	
			}
		%>
...
<table width="100%" border="0" cellpadding="2" cellspacing="0"  class="newtable" align="center"  >
    <tr>
      <td colspan="6" class="td_top" align="center" >电子设备卡片信息</td>
    </tr>
    <%if(!isCardInfo){ %>
    ...
    <tr>
      ...
      <%}else{ %>
      	<s:textfield name="body.zcname" id="zcname" maxlength="40" size="30"/>
      <%} %>
      </td>
    ...
    <tr>
      ...
      <%}else{ %>
      	<s:textfield name="body.saveadsress" size="30"/>
      <%}%>	
      </td>
    ...
    <tr>
      ...
      <td class="td_form02">
      <%if(isCardInfo){ %>
      <s:textfield name="body.username" id="username" disabled="true" />
      <%}else{ %>
      <s:textfield name="body.username" onblur="isString(this);" id="username" />
      <%} %>
      </td>
    ...
    <table width="100%" border="0" cellpadding="2" cellspacing="0" align="center"   class="newtable" >
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.ybyz" maxlength="20" size="30" id="ybyz" disabled="true"/>（元）
	      <%}else{ %>
	      		<s:textfield name="body.ybyz" onblur="checkFloat(this)" id="ybyz" maxlength="20" size="30"/>（元）
	      <%}%></td>
	      ...
	      <%}else{ %>
      		<s:textfield name="body.jzzb" id="jzzb" maxlength="20" onblur="checkFloat(this)" size="30"/>（元）
	      <%}%>
	      </td>
	      <td class="td_form01"> 使用年限 </td>
	      <td class="td_form02">
	      <%if(isCardModify||isCardInfo){%>
      			<s:textfield name="body.synx" maxlength="20" size="30" disabled="true"/>（年）
	      <%}else{ %>
	      		<s:textfield name="body.synx" onblur="checkFloat(this)" maxlength="20" size="30"/>（年）
	      <%}%></td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardModify||isCardInfo){%>
      			<s:textfield name="body.syyx" maxlength="20" size="30" disabled="true"/>（月）
	      <%}else{ %>
	      		<s:textfield name="body.syyx" onblur="checkFloat(this)" maxlength="20" size="30"/>（月）
	      <%}%></td>
	      <td class="td_form01">月折旧额</td>
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.yzje" id="yzje" maxlength="20" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      		<s:textfield name="body.yzje" id="yzje" onblur="checkFloat(this)" maxlength="20" size="30"/>（元）
	      <%}%></td>
	      <td class="td_form01">已计提月份</td>
	      <td class="td_form02">
	      <%if(isCardModify||isCardInfo){%>
      			<s:textfield name="body.yjtyf" maxlength="20" size="30" disabled="true"/>（月）
	      <%}else{ %>
	      		<s:textfield name="body.yjtyf" onblur="checkFloat(this)" maxlength="20" size="30"/>（月）
	      <%}%></td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.ljzj" id="ljzj" maxlength="20" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      		<s:textfield name="body.ljzj" id="ljzj" onblur="checkFloat(this)" maxlength="20" size="30"/>（元）
	      <%}%></td>
	      <td class="td_form01">净值</td>
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.jvalue" id="jvalue" maxlength="20" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      		<s:textfield name="body.jvalue" id="jvalue" onblur="checkFloat(this)" maxlength="20" size="30"/>（元）
	      <%}%></td>
	      <td class="td_form01">净额</td>
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.jevalue" id="jevalue" maxlength="20" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      		<s:textfield name="body.jevalue" id="jevalue" onblur="checkFloat(this)" maxlength="20" size="30"/>（元）
	      <%}%></td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.jcz" id="jcz" maxlength="24" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      		<s:textfield name="body.jcz" id="jcz" onblur="checkFloat(this)" maxlength="24" size="30"/>（元）
	      <%}%></td>
	      <td class="td_form01">净残值率</td>
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.jczl" maxlength="22" size="30" disabled="true"/>（%）
	      <%}else{ %>
	      		<s:textfield name="body.jczl" onblur="checkPresent(this);" maxlength="22" size="30"/>（%）
	      <%}%></td>
	      <td class="td_form01">月折旧率</td>
	      <td class="td_form02">
	      <%if(isCardModify||isMMergeBody||isCardInfo||isMSplitBody){%>
      			<s:textfield name="body.yzjl" maxlength="20" size="30" disabled="true"/>（%）
	      <%}else{ %>
	      		<s:textfield name="body.yzjl" onblur="checkPresent(this);" maxlength="20" size="30"/>（%）
	      <%}%></td>
	    </tr>
	</table>
    <table width="100%" border="0"  cellpadding="2" cellspacing="0" align="center"   class="newtable" >
	  ...
	  <table width="100%" border="0" cellpadding="2" cellspacing="0" align="center"   class="newtable" >
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.sbxlh" maxlength="20" size="30" disabled="true"/> 
	      <%}else{ %>
	      <s:textfield name="body.sbxlh" onblur="isString(this);" maxlength="20" size="30"/> 
	      <%} %>
	      </td>
	      <td class="td_form01"> 供应商 </td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.gys" maxlength="20" size="30" disabled="true" /> 
	      <%}else{ %>
	      <s:textfield name="body.gys" onblur="isString(this);" maxlength="20" size="30"/> 
	      <%} %>
	      </td>
	      <td class="td_form01"> 合同号码 </td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.hthm" maxlength="20" size="30" disabled="true"/> 
	      <%}else{ %>
	      <s:textfield name="body.hthm" onblur="isString(this);"  maxlength="20" size="30"/> 
	      <%} %>
	      </td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.bxq" maxlength="20" size="30" disabled="true"/> 
	      <%}else{ %>
	      <s:textfield name="body.bxq" onblur="isString(this);"  maxlength="20" size="30"/> 
	      <%} %>
	      </td>
	      <td class="td_form01">上门保修期</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.smbxq"  maxlength="20" size="30" disabled="true"/>  
	      <%}else{ %>
	      <s:textfield name="body.smbxq" onblur="isString(this);"  maxlength="20" size="30"/> 
	      <%} %>
	      </td>
	      <td class="td_form01">送修保修期</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.sxbxq" maxlength="20" size="30" disabled="true"/> 
	      <%}else{ %>
	      <s:textfield name="body.sxbxq" onblur="isString(this);" maxlength="20" size="30"/> 
	      <%} %>
	      </td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.cupxh" maxlength="20" size="30" disabled="true"/>
	      <%}else{ %>
	      <s:textfield name="body.cupxh" onblur="isString(this);" maxlength="20" size="30"/>
	      <%} %>
	      </td>
	      <td class="td_form01">外置硬盘</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.wzyp" maxlength="20" size="30" disabled="true"/>
	      <%}else{ %>
	      <s:textfield name="body.wzyp" maxlength="20" size="30"/>
	      <%} %>
	      </td>
	      <td class="td_form01">内置硬盘</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.nzyp" maxlength="20" size="30" disabled="true"/>
	      <%}else{ %>
	      <s:textfield name="body.nzyp" onblur="isString(this);" maxlength="20" size="30"/>
	      <%} %>
	      </td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.xsq" maxlength="24" size="30" disabled="true"/>
	      <%}else{ %>
	      <s:textfield name="body.xsq" onblur="isString(this);" maxlength="24" size="30"/>
	      <%} %>
	      </td>
	      <td class="td_form01">硬盘型号</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.qpxh" maxlength="22" size="30" disabled="true"/>
	      <%}else{ %>
	      <s:textfield name="body.qpxh" onblur="isString(this);" maxlength="22" size="30"/>
	      <%} %>
	      </td>
	      <td class="td_form01">内存</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.nsave" maxlength="20" size="30" disabled="true"/>
	      <%}else{ %>
	      <s:textfield name="body.nsave" onblur="isString(this);" maxlength="20" size="30"/>
	      <%} %>
	      </td>
	    </tr>
	</table>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="center"   class="newtable" >
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.pgqyz" maxlength="20" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      <s:textfield name="body.pgqyz" onblur="isString(this);" maxlength="20" size="30"/>（元）
	      <%} %>
	      </td>
	      <td class="td_form01">评估前累计折旧</td>
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.pgqljzj" maxlength="20" size="30" disabled="true"/>（元）
	      <%}else{ %>
	      <s:textfield name="body.pgqljzj" onblur="isString(this);" maxlength="20" size="30"/>（元）
	      <%} %>
	      </td>
	    </tr>
	    <tr>
	      ...
	      <td class="td_form02">
	      <%if(isCardInfo){ %>
	      <s:textfield name="body.yxtlry" maxlength="15" size="30" disabled="true" />
	      <%}else{ %>
	      <s:textfield name="body.yxtlry" onblur="isString(this);" maxlength="15" size="30"/>
	      <%} %>
	      </td>
	      <td class="td_form01">&nbsp;</td>
	      <td class="td_form02">&nbsp;</td>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" align="center"   class="newtable" >
		<tr>
	      ...
	      <%}else{ %>
	      <s:textarea name="body.bzsm" cols="150" rows="4"></s:textarea>
	      <%} %>
	      </td>
	    </tr>
	</table>
