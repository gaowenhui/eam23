...
<table width="95%" border="0" cellpadding="2" cellspacing="0"  class="newtable" align="center"  >
    ...
    <tr>
      	<td colspan="6" class="td_form01" align="center" >
      		...
      		<input type="button" class="buttonface" value="选择资产" onclick="findZC();">
      	</td>
    </tr>
    <tr>
    ...
    <tr>
		...
		<td class="td_form02" colspan="5"><s:textarea name="body.mainyt" id="mainyt" cols="150" rows="4" readonly="true"></s:textarea></td>
    </tr>
</table>
...
<table width="95%" border="0" cellpadding="2" cellspacing="0" align="center"   class="newtable" >
	<tr>
	    ...
	    <td class="td_form02"><s:textfield name="body.jzzb" id="jzzb" maxlength="20" size="30" readonly="true"/>（元）</td>
    ...
    <tr>
	    ...
	    <td class="td_form02"><s:textfield name="body.jevalue" id="jevalue" maxlength="20" size="30" readonly="true"/>（月）</td>
    </tr>
    <tr>
...
// 改完减少方式后点击提交保存body
function saveBody(){
	if(document.getElementById("chooseData").value==""){
		alert("请选择资产！");
		return false;
	}
	...
	myform.action = '<%=basePath%>kpgl/cardBody_saveBody.do?sheetType=allocate&isModifyBody=false';
	myform.submit();
}
...
function updateBody(){
	...
	myform.action='<%=basePath%>kpgl/cardBody_saveBody.do?sheetType=allocate&isModifyBody=true';
	myform.submit();
}
...
function getBodyInfo(id){
	...
	var headId = '<%=headId%>';
	var checkAllocate = "<%=basePath%>kpgl/cardBody_checkAllocate.do";
	$.post(checkAllocate,{cardId:"" + id + ""}, function(data){
		if(data=='no1'){
			alert('所选卡片挂接自用资产，不能调配！');
		}else if(data=='no2'){
			alert('所选卡片挂接资产正在处置中，不能调配！');
		}else{
			myform.action = "<%=basePath%>kpgl/cardBody_getBodyInfo.do?cardId="+cid+"&sheetType="+sheetType+"&headId="+headId;
			myform.submit();
		}
	});
}
...
// 资产选择后回调方法
function jyzcinfo(zcid){
	var sheetType = '<%=sheetType%>';
	var headId = '<%=headId%>';
	myform.action = "<%=basePath%>kpgl/cardBody_getBodyInfo.do?sheetType="+sheetType+"&headId="+headId+"&zcid="+zcid;
	myform.submit();
}
