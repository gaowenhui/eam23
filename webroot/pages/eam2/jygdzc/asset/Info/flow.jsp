		...
		<script type="text/javascript">
			...
			var title = document.getElementById("title").value;
				...
				<%if("change".equals(sheettype2)){%>	
					url:"<%=basePath%>jygdzc/AssetManageBody_queryWorkFlow.do?sheettype2=change&zcId=${zcId}",
				<%}else{%>
		...
		$(function(){
			...
			$(window).bind('resize', function(){
				...
				jQuery("#endtask_list").jqGrid({	
					<%if("change".equals(sheettype2)){%>	
						url:"<%=basePath%>jygdzc/AssetManageBody_queryWorkFlow.do?sheettype2=change&zcId=${zcId}&sheettype=${sheettype}",
					<%}else{%>
