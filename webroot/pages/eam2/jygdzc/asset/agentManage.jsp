...
%>
<!-- ==========================  权限控制添加     ======================================== -->
<r:controlfile file="/pages/eam2/jygdzc/asset/agentManage.ctl.xml"></r:controlfile>
<!-- ================================================================================== -->
<!-- ==========================工作流属性初始化=========================================== -->
<%@ include
	...
	<%
		...
		String isJbrcl = String.valueOf("业务管理部门经办人处理".equals(workflowVO.taskName));
	%>
<!-- ================================================================================== -->
<html>
<head>
  ...
  <!-- ============================================================================ -->
   function createBook4Card(){
	...
	<%
		if (workflowVO.taskName.equals("建银实业综合处理")) {
	%>
		if("业务管理部门综合处理"==outCome||"业务管理部门经办人处理"==outCome){
			if("0"=="<%=statusjysy%>"){
		      	alert("正文未新建");
	    	 	return false;
        	}else{
	...
	//（处理）新建正文
		...
		function openzhengwenJysy(){
			wordTemplate('blank_Template_jysy', document.getElementById("myform"),false,true, "日常管理（代理机构）正文"); 
			//修改按钮名称为“处理正文”
		...
		function viewzhengwen(){
			wordTemplate('bank_Template', document.getElementById("myform"),true,false,"日常管理（代理机构）正文"); 
		}
		//查看正文
		function viewzhengwenjysy(){
			wordTemplate('blank_Template_jysy', document.getElementById("myform"),true,false,"日常管理（代理机构）正文"); 
		}
		//合成正文
		function hechengzhengwen(){
			...
			MergeWordStart('manager_jic_jysy_gzlxdht','bank_Template','manager_gzlxdbj' , document.getElementById("myform"), "日常管理（代理机构）正文",'hechengHead()','hechengBottom()');
		...
		function hechengzhengwenjysy(){
		    ...
		    //  return false;
		   // }
			MergeWordStart('manager_jysy_gzlxdht','blank_Template_jysy','manager_gzlxdbj' , document.getElementById("myform"), "日常管理（代理机构）正文",'hechengHeadJysy()','hechengBottomJysy()');
			window.parent.removeObj("#hczwButton");
			isMergejysy = true;
			<%
				if (workflowVO.taskName.equals("建银实业综合处理")) {
...
<s:hidden name="sheettype" value="%{#request.sheettype}" id="sheettype" />
<s:hidden  name="head.sheettype" value="%{#request.sheettype}" />
<s:hidden name="head.lcmc" id="lcmc" value="日常管理(代理机构)" />
...
<!-- =========================================================================== -->
      ...
      <div align="center" nowrap="nowrap">
        <span class="STYLE7 STYLE1">中国建银投资有限责任公司日常管理(代理机构)<%@include file="/pages/rdp4j/workflow/task/workflow_title_include.jsp"%></span>
      </div>
      <br/>
<!-- ==========================工作流信息项======================================= -->
<%@ include
...
<!-- =========================================================================== -->
    ...
    <tr>
	  ...
	  <td class="td_zxy02">
	    <r:select list="#{'0':'确权审批','1':'更新改造','2':'业务维修','3':'其他事项审批'}"  onclick="clickywlxIndex();"  name="head.ywlx" id="ywlx" onchange="getIndex()" />
	  </td>
	  ...
	  <tr>
		...
		<td class="td_zxy02" valign="middle">
			<e:select parRefKey="department" headerKey="" headerValue="" parentId="438" list="#{}" name="head.region"   id="jbh"  onclick="clickIndex();"  onchange="change()"/>	
		</td>
...
function getIndex() {
  ...
  function addValue(gr){
		...
		function send2Erp(){  
			var headId = document.getElementById('headId').value; 
			var url = "<%=request.getContextPath()%>/jiekou/send2ERP4RCGL_requestExecute.do";
			if(confirm("是否发送ERP？")){
				$.post(url,{headId:"" + headId + ""}, function(data){
					if(data !=""){
						alert(data);
...
jQuery("#assetlist1").jqGrid({				
    ...
    {name: 'sksynx',index: 'sksynx',align: "center",width:100}, 
    {name: 'sqje',index: 'sqje',align: "center",width:100}, 
    {name: 'sdje',index: 'sdje',align: "center",width:100}],
...
%>	
		var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_modifyBody.do?sfsjfy='+sfsjfy+'&headId='+headId+'&jbh='+yjfh+'&sheettype2=bmldsh&sheettype=manageInfo&zcId='+gr;
<%
	}else{
%>	
		var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_modifyBody.do?sfsjfy='+sfsjfy+'&headId='+headId+'&jbh='+yjfh+'&sheettype=manageInfo&zcId='+gr;
<%	
	}
%>		
...
<%
	if(workflowVO.taskName.equals("拟稿")){
%>	
	var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_newBody.do?sfsjfy='+sfsjfy+'&ywlx=manage&jbh='+yjfh+'&sheettype2=bmldsh&headId='+headId+'&sheettype=manageInfo&isAdd=true&bmbh='+bmbh;
<%
	}else{
%>	
	var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_newBody.do?sfsjfy='+sfsjfy+'&ywlx=manage&jbh='+yjfh+'&headId='+headId+'&sheettype=manageInfo&isAdd=true&bmbh='+bmbh;
<%	
	}
%>
	url = url  + "&id=" + new Date();//这个id使每次的url不同，是防止缓存加的时间参数，
...
jQuery("#assetlist2").jqGrid({				
	...
	colModel: [
	{name: 'id',index: 'id',align: "center",hidden:true}, 
	{name: 'zcbh',index: 'name',align: "center",width:100}, 
	{name: 'zcmc',index: 'name',align: "center",width:100}, 
	{name: 'saveadsress',index: 'saveadsress',align: "center",width:100},
...
%>
	if("3"==ywlx){
		var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_newBody.do?sfsjfy='+sfsjfy+'&ywlx=else&sheettype2=bmldsh&headId='+headId+'&jbh='+yjfh+'&sheettype=manageInfo&isAdd=true&bmbh='+bmbh;
	}else{
		var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_newBody.do?sfsjfy='+sfsjfy+'&ywlx=manage&sheettype2=bmldsh&headId='+headId+'&jbh='+yjfh+'&sheettype=manageInfo&isAdd=true&bmbh='+bmbh;
	}
<%
	}else{
%>
	if("3"==ywlx){
		var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_newBody.do?sfsjfy='+sfsjfy+'&ywlx=else&headId='+headId+'&jbh='+yjfh+'&sheettype=manageInfo&isAdd=true&bmbh='+bmbh;
	}else{
		var url = '<%=request.getContextPath()%>/jygdzc/AssetManageBody_newBody.do?sfsjfy='+sfsjfy+'&ywlx=manage&headId='+headId+'&jbh='+yjfh+'&sheettype=manageInfo&isAdd=true&bmbh='+bmbh;
	}
<%	
	}
%>
